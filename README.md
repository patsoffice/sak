...
- `sak nix flake-show` / `sak nix store-info` / `sak nix eval` / `sak nix registry-list` / `sak nix profile-list` instead of `nix flake show` / `nix store info`/`ping` / `nix eval` (pure evals only) / `nix registry list` / `nix profile list` (more nix read commands arrive in follow-ups)
