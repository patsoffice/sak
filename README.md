...
- `sak linux cpuinfo|meminfo|mounts|loadavg|uptime|sysctl` instead of `awk`-ing `/proc/cpuinfo`, `/proc/meminfo`, the mount table, `/proc/loadavg`, `/proc/uptime`, or `sysctl -a` (Linux-only; more `/proc` parsers arrive in follow-ups)
