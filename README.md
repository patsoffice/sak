...
| --- | --- | --- |
| `k8s` | yes | The `k8s` domain (`contexts`, `kinds`, `get`, `images`, `env`, `schema`, `restarts`, `failing`, `pending`, `not-ready`, `events`, `describe`, `logs`) and the `kube` / `k8s-openapi` / `tokio` / `http` dependencies needed to talk to a live cluster. |
...
- `sak nix flake-show` / `sak nix store-info` / `sak nix eval` / `sak nix registry-list` / `sak nix profile-list` / `sak nix references` / `sak nix derivation-show` / `sak nix path-info` instead of `nix flake show` / `nix store info`/`ping` / `nix eval` (pure evals only) / `nix registry list` / `nix profile list` / `nix-store --query --references`/`--referrers`/`--requisites` / `nix derivation show` / `nix path-info` / `nix-store --query --info`/`-S` (more nix read commands arrive in follow-ups)
