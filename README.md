...
- `sak linux cpuinfo|meminfo|mounts|loadavg|uptime|sysctl|process` instead of `awk`-ing `/proc/cpuinfo`, `/proc/meminfo`, the mount table, `/proc/loadavg`, `/proc/uptime`, `sysctl -a`, or `/proc/<pid>` (Linux-only; more `/proc` parsers arrive in follow-ups)
