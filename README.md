...
Commands are organized by domain. Current domains: `fs` (filesystem), `git` (repository), and `json`, with more planned (e.g., `csv`, `k8s`).
...
- **Minimal dependencies** — Runtime dependencies: `clap`, `globset`, `walkdir`, `regex`, `anyhow`, `git2`, `serde`, `serde_json`.
...
| `serde` / `serde_json` | JSON parsing and value model |
