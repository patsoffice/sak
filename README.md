...
- `sak nix flake-show` / `sak nix store-info` / `sak nix eval` instead of `nix flake show` / `nix store info`/`ping` / `nix eval` (pure evals only; more nix read commands arrive in follow-ups)
