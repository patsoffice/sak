...
- `sak gh pr-list` / `sak gh issue-list` / `sak gh run-list` / `sak gh release-list` / `sak gh api <endpoint>` instead of `gh pr list` / `gh issue list` / `gh run list` / `gh release list` / `gh api` / `curl`ing the GitHub REST or GraphQL API for reads
