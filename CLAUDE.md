...
- `sak json query|exists|keys|flatten|paths|grep|schema|type|validate|diff` for `*.json`
- `sak config query|exists|keys|flatten|paths|grep|schema|type|validate|diff` for TOML, YAML, plist
