...
- `sak nix flake-show` / `sak nix store-info` / `sak nix eval` / `sak nix registry-list` / `sak nix profile-list` / `sak nix references` / `sak nix derivation-show` / `sak nix path-info` instead of `nix flake show` / `nix store info`/`ping` / `nix eval` / `nix registry list` / `nix profile list` / `nix-store --query --references`/`--referrers`/`--requisites` / `nix derivation show` / `nix path-info` (more nix read ops arrive in follow-ups)
