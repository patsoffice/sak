...
static QUICK_START: LazyLock<String> = LazyLock::new(|| {
  ...
  sak json table .issues -f id,title in.json  Array of objects -> TSV rows
  ...
  sak k8s not-ready kustomization -A          Objects whose Ready condition is not True
...
enum Command {
    ...
    /// Nix store / flake / profile inspection (read-only, shells out to nix)
    #[command(subcommand)]
    Nix(nix::NixCommand),
