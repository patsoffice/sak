...
//! enough to test exhaustively, and the prom domain deliberately keeps its
//! dependency surface minimal (ureq + rustls only). This mirrors the
//! hand-rolled `parse_since` in `src/k8s/logs.rs`, extended with `w`.
